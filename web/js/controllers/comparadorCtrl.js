angular
  .module("antares")
  .controller("comparadorCtrl", function ($scope, antaresAPI, graficoAPI) {
    $scope.mostrarGrafico = true;

    var dados = null;
    $scope.pesquisar = function (primeiraOpcao, segundaOpcao) {
      antaresAPI
        .ajaxPost("src/com/wjuan/antares/requests/requestComparador.php", {
          c1: primeiraOpcao,
          c2: segundaOpcao,
        })
        .then(
    ...
    var dataSets = function (sentimentos) {
      ...
      for (var i = 0; i < 2; i++) {
        ...
        var dataSet = {
          ...
          data: [
            sentimentos[i].dados.porcentagemPositivo,
            sentimentos[i].dados.porcentagemNegativo,
          ],
